import {
    EventsInterface
} from './../events/interfaces/events.interface';
import {
    createTicket
...
import {
    TicketCategoriesInterface
} from 'src/ticket-categories/interfaces/ticket.categories';

@Injectable()
export class TicketsService {
    constructor(@Inject('Ticket_MODEL') private ticketsModel: Model < TicketsInterface > ,
        @Inject('Event_MODEL') private eventsModel: Model < EventsInterface > ,
        @Inject('TicketCategories_MODEL') private ticketCategoriesModel: Model < TicketCategoriesInterface >
    ...
    async getrelatedEvents(id: string) {
        return await this.eventsModel.findOne({
            '_id': id
        }).populate('venue').populate('eventCategory').populate('tickets').exec();
    }
    ...
    async createtickets(event_id: string, createticketsDto: createTicket) {
        ...
        for (var i = 0; i < (await event).tickets.length; i++) {
            var eventsAvailableTicketCategories = await this.ticketCategoriesModel.findOne({
                '_id': (await event).tickets[i]
            });
